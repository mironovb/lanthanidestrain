...
\paragraph{Where 3D structure enters.}
The optimised complexes contribute only through the shape. Blended into the shape channel at $w = 0.35$, the encoder raises the score from $+0.318$ to $+0.326$ (eight CatBoost seeds, a 32-seed encoder ensemble). The same geometry summarised as 84 descriptor columns and handed to a flat model takes it from $+0.278$ to $-0.022$. The gain is smaller than the seed spread of a single encoder run (sd 0.0285), so we claim no more than this. Structure helps only where the level is held fixed.

\section{What 3D structure can add, and how to check a structure set before training}
\label{sec:geom}
Eight encoders trained on the same complexes, differing in architecture, neighbourhood cutoff and loss, make almost the same predictions. Two disjoint eight-seed halves of one configuration agree at $r = 0.990$; ensembles of two different configurations agree at $0.986$; the eight prediction vectors have an effective rank of 1.05.

The reason is in the parameter file. GFN2-xTB fits cerium and lutetium and interpolates every lanthanide parameter linearly in $Z$ between them (largest residual $5.7 \times 10^{-7}$). An optimised metal--donor shell can therefore respond to the metal through a single scalar, and the tabular model already has that scalar as a feature. For 71 ligands we substituted all fifteen lanthanides into one relaxed complex, re-optimised under each Hamiltonian with the same binary and protocol, and regressed the mean metal--donor distance on the Shannon radius; the slope \cL{} is 1 when the complex contracts exactly as the ionic radii do (Figure~\ref{fig:main}c). GFN2-xTB gives $\cL = 0.405 \pm 0.145$. g-xTB \citep{gxtb}, with the f electrons in the valence, gives $1.078 \pm 0.094$ and is closer to 1 on 70 of the 71 ligands. Ln-xTB \citep{lnxtb2026}, an element-by-element refit of the GFN2 lanthanide parameters, recovers the mean ($1.081 \pm 0.214$) but not a smooth series: a straight line explains 43\,\% of its metal dependence against 82\,\% for g-xTB, and it has a 0.10~\AA{} dip at gadolinium that every ligand shares.

Getting the contraction right does not, on its own, give a model anything new to learn. g-xTB geometries in place of GFN2-xTB change the score by $+0.005$ ($p = 0.68$, eight paired seeds); rebuilding the complexes in correspondence or relaxing them in solvent makes it worse (Appendix~\ref{app:controls}). Under all three Hamiltonians the ligand-specific part of the compliance does not correlate with measured selectivity ($r = 0.11$, $-0.02$ and $0.17$ on 44 ligands); what the Hamiltonian changes is a profile common to every ligand. The benchmark is therefore the useful product: run on a candidate set of structures before any encoder is trained, it shows whether they carry more than one number about the metal.
