...
The 3D gain is small against seed variability, and the structure set omits, by construction, the quantities that could differ between ligands: the coordination number follows a rule of the metal alone, 89\,\% of the complexes are the bare Ln$^{3+}$ cation rather than the extracted species, and only geometries, not binding energies, are used. These are the natural next inputs for the shape term, and the compliance benchmark is a cheap check of whether a candidate structure set carries more than the ionic radius. No ceiling is identified: separations reproduce to about 0.16 on a spread of 0.22, but 70\,\% of scored pairs have no replicate, and within one extractant the model's series shape is weak (median per-extractant $R^2 = 0.09$). What it predicts well is which ligands are selective and where their large separations lie. The step that mattered was asking the model the right question: the shape of a series, not its level.
