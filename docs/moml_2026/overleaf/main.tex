...
\label{sec:geom}
The optimised complexes contribute to the model only through the shape term. Adding the block-centred prediction of the 3D encoder to the tabular shape with weight $w = 0.35$ increases the score from $+0.318$ to $+0.326$ (eight CatBoost seeds; 32-seed encoder ensemble). This increase is smaller than the standard deviation between seeds of a single encoder run (0.0285). When the same geometries are instead converted to 84 descriptor columns and added to a model that predicts \logD{} directly, the score decreases from $+0.278$ to $-0.022$. Structural information is therefore useful only within the shape term.

The small size of the gain is explained by the information content of the geometries. Eight encoders with different architectures, neighbourhood cutoffs and loss functions were trained on the same complexes. Their predictions are nearly identical: two disjoint eight-seed halves of one configuration correlate at $r = 0.990$, ensembles of two different configurations at $0.986$, and the eight prediction vectors have an effective rank of 1.05. In GFN2-xTB the parameters of the lanthanides are obtained by linear interpolation in $Z$ between fitted values for Ce and Lu. Along the series, the optimised metal--donor distances therefore vary with the metal through a single scalar, which is equivalent to the ionic radius already present in the tabular features.

The accuracy of this scalar was quantified for 71 ligands. All fifteen lanthanides were substituted into one relaxed complex per ligand, the structures were re-optimised with each Hamiltonian under identical settings, and the mean metal--donor distance was regressed on the Shannon radius. The slope \cL{} equals 1 when the complex contracts exactly as the ionic radii (Figure~\ref{fig:main}c). GFN2-xTB yields $\cL = 0.405 \pm 0.145$. g-xTB \citep{gxtb}, with explicit f electrons, yields $1.078 \pm 0.094$ and is closer to 1 for 70 of the 71 ligands. Ln-xTB \citep{lnxtb2026}, a re-parameterisation of the GFN2-xTB lanthanide parameters element by element, reproduces the mean slope ($1.081 \pm 0.214$) but not the regularity of the series: a linear fit explains 43\,\% of its metal dependence, compared with 82\,\% for g-xTB, and all ligands show a 0.10~\AA{} deviation at gadolinium.

An accurate contraction does not by itself improve the prediction. Replacing the GFN2-xTB geometries with g-xTB geometries changes the score by $+0.005$ ($p = 0.68$, eight paired seeds); rebuilding the complexes in correspondence or relaxing them in solvent lowers it (Appendix~\ref{app:controls}). For all three Hamiltonians the ligand-specific compliance is uncorrelated with the measured selectivity ($r = 0.11$, $-0.02$ and $0.17$ for 44 ligands), and the differences between Hamiltonians consist of a profile common to all ligands. The compliance benchmark is inexpensive and can be applied to any set of generated structures before an encoder is trained on them.
