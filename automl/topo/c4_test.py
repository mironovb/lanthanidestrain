...
ART = REPO / "automl/artifacts/topo_c4"
REPORTS = REPO / "automl/reports"
OUT_CELLS = REPORTS / "c4_cells.csv"
OUT_TEST = REPORTS / "c4_test.csv"

SEEDS = [7, 11, 23, 37]
CONFIRM_SEEDS = [101, 103, 107, 109, 113, 127, 131, 137, 139, 149, 151, 157]
MIN_GAIN = 0.005
N_LOOKS = 29           # 26 from campaign 3 + one per new arm

CELLS: dict[str, dict] = {
    "shipped": {"geometry": "shipped"},
...
def main() -> int:
    ...
    ctl = _score(restrict(cells["control"], tune), BINNED)[0]
    print(f"\n=== screening on the TUNE half only (control = {ctl:+.4f}) ===")
    print(f"  {'cell':5s} {'axis':16s} {'tune binned':>12s} {'vs control':>9s} "
    ...
    for name in CELLS:
        ...
        rows.append({"cell": name, "axis": AXIS.get(name, ""),
                     ...
                     "gain_vs_control": gain, "tune_adj_strict": ts,
                     "tune_r2_overall": r2})
  ...
  making the structural representation depend on the medium.""")
    ...
    name = str(best["cell"])
    print(f"  winner on tune: {name} at {float(best['gain_vs_control']):+.4f} "
          f"(> +{MIN_GAIN:.3f})")
    print(f"""
