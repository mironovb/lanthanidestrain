...
The gain is limited by the geometries, which vary with the metal through a single number. Eight encoders with different architectures, cutoffs and losses give nearly identical predictions ($r = 0.990$ between disjoint eight-seed halves; effective rank 1.05 of the eight prediction vectors). In GFN2-xTB the lanthanide parameters are linear interpolations in $Z$ between Ce and Lu, so the optimised metal--donor distances follow the ionic radius, which the tabular features already contain. The contraction benchmark (Figure~\ref{fig:main}c; 71 ligands, fifteen lanthanides substituted into one relaxed complex per ligand and re-optimised with each Hamiltonian) gives a slope of the mean metal--donor distance against the Shannon radius of $0.405 \pm 0.145$ for GFN2-xTB, $1.078 \pm 0.094$ for g-xTB \citep{gxtb}, and $1.081 \pm 0.214$ for Ln-xTB \citep{lnxtb2026}, whose series is irregular (linear-fit $R^2$ 0.43 against 0.82 for g-xTB). Replacing the GFN2-xTB geometries with g-xTB geometries changes the score by $+0.005$ ($p = 0.68$, eight paired seeds; Appendix~\ref{app:controls}), and the ligand-specific part of the slope is uncorrelated with measured selectivity under all three Hamiltonians ($r = 0.11$, $-0.02$, $0.17$; 44 ligands).
...
The 3D term can only grow with structures that differ between ligands in ways the present set excludes by construction. Three extensions follow directly: assigning the coordination number per ligand rather than by a rule of the metal, so that a change of coordination number along the heavy half of the series can appear; building the neutral extracted species with its counter-ions instead of the bare Ln$^{3+}$ cation, which is what 89\,\% of the present complexes are; and computing the energy of accommodating each ion, not only its geometry. Each candidate structure set can be screened with the compliance benchmark before an encoder is trained on it, and the heavy half of the series, where every current model has little skill, is where the gain would be measured.

\paragraph{Limitations and conclusion.}
Three limitations should be kept in mind. The contribution of the 3D term is smaller than the variation between seeds of a single encoder run. An upper bound on the attainable score cannot be given: measured separations agree to about 0.16 log units between independent condition sets, against a spread of 0.22, but 70\,\% of the scored pairs come from cells that were measured only once. Within a single extractant the predicted shape of the series is weak (median $R^2 = 0.09$ per extractant); the model is reliable for which ligands are selective and for where their largest separations occur, not for the fine structure of one ligand's series. Within these limits, the record of published extraction data is sufficient to predict adjacent-lanthanide selectivity for unseen ligands, and the decisive modelling choice was to predict the shape of a series separately from its level.
