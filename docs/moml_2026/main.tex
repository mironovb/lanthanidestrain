...
{Bogdan Mironov \\
 ...
 \email{daulet_d0806@hbalm.nis.edu.kz}\And
...
\section{Model}
\label{sec:model}
The tabular features are RDKit descriptors and 2048-bit Morgan fingerprints of the extractant \citep{rogers2010}, the metal's atomic number, position in the series and Shannon radius, and the 64 condition columns, 746 in total. The learner is CatBoost \citep{catboost2018} with a quantile loss ($\alpha = 0.6$), depth 9 and feature subsampling 0.3, chosen on the adjacent-pair score. Trained on \logD{} directly it reaches $\adjR = +0.268$.
...
\paragraph{The 3D gain is small.}
The blend improves on the level/shape model by $+0.008$ ($+0.318 \to +0.326$; eight CatBoost seeds, a 32-seed encoder ensemble, $w$ chosen by a nested criterion). That is below the seed-to-seed spread of a single encoder run (sd 0.0285), which is why the encoder enters only as a large ensemble and why we claim no more than a small effect. Geometry helps when it can only move the shape; as 84 derived columns in a flat model it takes the score from $+0.278$ to $-0.022$.
