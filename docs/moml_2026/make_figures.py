...
def fig2b(comp, ch=None):
    ...
    ax.set_xticklabels([f"{n}\nmean {v.mean():.2f}" for (_, n), v in zip(HAM, data)])
    ax.tick_params(axis="x", length=0)
    ax.set_xlim(-0.5, len(HAM) - 0.5); ax.set_ylim(0, 2.0)
...
def fig3(ch=None):
    ...
    # the two extrapolations, drawn to twice the full size and named at their ends
    xx = np.logspace(np.log10(pts.n.min() * 0.85), np.log10(2.0 * n_full), 200)
    hy, pw = F["hyperbolic"], F["power"]
    y_h = hy["R_inf"] - hy["k"] / xx
    y_p = pw["R_inf"] - pw["k"] * xx ** (-pw["c"])
    ax.plot(xx, y_h, color=col, lw=1.0, ls="-", zorder=3)
    ax.plot(xx, y_p, color=col, lw=1.0, ls=":", zorder=3)
    ax.text(xx[-1] * 1.04, y_h[-1], "hyperbolic", va="center", ha="left", color=col)
    ax.text(xx[-1] * 1.04, y_p[-1], "power law", va="center", ha="left", color=col)
    ax.axvline(n_full, ls="--", lw=0.7, color=fs.OI["black"], zorder=1)
    ax.text(n_full, 0.375, "all", ha="center", va="bottom", color=fs.OI["black"])
    ax.set_xscale("log"); ax.set_xlim(pts.n.min() * 0.8, 2.0 * n_full * 1.9)
    ...
    ax.text(0.97, 0.03, f"{len(R)} runs", transform=ax.transAxes, ha="right", va="bottom")
    letter(ax, ch)
